module.exports = {
  ...
  parserOptions: {
    ...
    warnOnUnsupportedTypeScriptVersion: false, // Silence warnings from @typescript-eslint saying we're using a newer version of TypeScript that may not support
  },
  settings: {
    react: {
  ...
  rules: {
    ...
    '@typescript-eslint/ban-ts-comment': 'off', // We're learning TypeScript right now, so lets have an opt-out strategy so we can push code without needing to fix it.
